...
get_model_diagnostic_tests = function(models, model_names){
  ...
  for (idx in 1:n_mod){
    diagnostics_metrics_df[model_names[idx], "BP_P_Value"] = bptest(models[[idx]])$p.value
    diagnostics_metrics_df[model_names[idx], "BP_DEC"] = get_bp_decision(models[[idx]])
    diagnostics_metrics_df[model_names[idx], "SW_P_VALUE"] = shapiro.test(sample(resid(models[[idx]]), 5000))$p.value
    diagnostics_metrics_df[model_names[idx], "SW_DEC"] = get_sw_decision(models[[idx]])
  }
...
get_sw_decision = function(model, alpha=0.05) {
  decide = unname(shapiro.test(sample(resid(model), 5000))$p.value < alpha)
  ifelse(decide, "Reject", "Fail to Reject")
}
...
plot_fitted_resid = function(model, model_name, stitle) {
...
Our final step was to attempt variable transformations to attempt to better capture the variance in the response variable. We transformed predictors `population` and `total_rooms` to be normalized with predictor `households`. We think derived variables 'rooms_per_hh' and 'pop_per_hh' should add predictive power to the model because of the following reasons. 
- 'rooms_per_hh': houses with higher number of rooms per household is likely going to be larger having a positive correlation with housing prices 
- 'pop_per_hh': block with bigger houses will attract larger families (population_per_household)

<!TODO: Avi, can you add about your reasoning here why we need additional variables transformed. The results are showing that this model is over-fitting?>
...
We also ran model diagnostic tests to check linear regression assumptions with the help of Breusch-Pagan Test and Shapiro-Wilk Test. 
- Breusch-Pagan Test decision for all the models was to 'Reject the null hypothesis' meaning there is some heteroscedasticity in the models 
- Shapiro-Wilk test shows that we could "Reject the null hypothesis' for al the models. i.e., model error is not following a normal distribution. 

```{r}
get_model_diagnostic_tests(cahoudta_models, cahoudta_model_names)
```

We are showing the model diagnostic plots for our top 2 models 1) cahoudta_full_add_model and 2) cahoudta_add_bckwd_bic_model. As it can be see, with log transformation of response we could reduce the issue of heteroscedasticity and error not following normal distribution. However, this is not entirely gone as it can be seen from the plots below. 
...
<!TODO: Avi's helper methods related to results should go under "METHODS" Section. Call the driver function here.>
<!TODO: Compare the two models 'cahoudta_full_add_model' and 'cahoudta_add_bckwd_bic_model'>
<!TODO: Residuals vs Fitted Plot side by side for two models>
<!TODO: bptest() for all models - tabular format for two models>
<!TODO: Q-Q plot for each model side by side for two models>
<!TODO: Shapiro Wilk test for all models - tabular format - for the two models>
<!TODO: [Wishlist] Use pairs plot from "Models Selection" section as reference to build a magnified version
        for each variable vs response under results section. Have a driver function which plots 6 scatterplots
        and a box plot for ocean_proximity. Use log of response.>
...
- Extracting meaningful Geo-Spatial insights / predictor variables based on 'Latitude' & 'Longitude' data: Houses within 50 miles radius of largest cities in California would likely have higher prices compared to houses which are outside 50 miles radius. Our team ideated extracting variables using Geo-Spatial libraries and methods such as  'Haversine Distance' - formula to calculate distance between 2 points on a sphere based on lat / long.  Another alternative was to simply use Euclidian Distance  based on lat and long information of houses and large cities to tag houses within 50 miles radius. In the interest of time we could not create these variables but we certainly would like to try including these Geo-Spatial variables in our model. 

- Looking at the response variable histogram, it seems that the median house prices are capped at the top. We would like to further explore the impacts of this in response variable. or maybe limit the model usage for houses with very high value 

- Our 9th model - cahoudta_2int_txfd_model - has the highest adjusted r-square although it suffers from over-fitting issue. We would like to investigate this further with the objective of mitigating over-fitting withough sacrificing adjusted r-square performance. 

- In our dataset 'total_bed_rooms' column had missing values. Given this column was highly correlated with 'total_bedrooms' and number of missing value being low - we safely deleted 'total_bedrooms' variable from the modeling exercise. In future, we would like to treat 'total_bedrooms' with missing value imputation to find out whether it has stronger signal than 'total_rooms' variable 

- We see some from BP Test result and 'Fitted vs. Residual' plot that the residuals still have some heteroscedasticity. In future, we would like to further reduce the issue by trying tranformation of predictor varibales 

- Our Q-Q plot for top 2 models does mostly look good, however towards the lower edge we see some diversion from the line. Time permitting, we would investigate more going forward to resolve the normality of error term issue 

<!TODO: Avi to add details around thought process of using variable transformations such as converting lat/lon to
        regions/counties. And explain why we couldn't do that in our project because of time constraints. 
        If deemed necessary also add details around "euclidean" distance idea and reasoning behind exploring that
        option.>
