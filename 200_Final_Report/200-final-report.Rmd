...
We also generated histograms for all relevant predictors. The most important finding discovered while performing EDA was the long tail of our outcome variable `median_house_value`.
...
At this point, we have concluded the prerequisite data cleaning and can move onto model selection. Our final data set used for modelling has `r nrow(ca_housing_data)` rows and `r ncol(ca_housing_data)` columns. The summary statistics can be found below.
