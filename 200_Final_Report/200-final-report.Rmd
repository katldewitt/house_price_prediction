...
# function to plot fitted vs. residual plot to test homoscedasticity - 
# uniform variation in error plotted against the fitted value or response  
...
plot_qq = function(model, model_name) {
  ...
  qqnorm(resid(model), col = green_color, pch = 20,  
...
Looking at the response variable histogram, it seems that median house prices are capped at the higher end instead of a long tail. We would like to further explore the impacts of this capping in our models. To address this, possibly deleting observations with very high median house prices and build the model only on median housing prices which are not capped. This data modification may limit the use of the model for very high housing prices but will help with overall accuracy (& R-square) for non-capped housing price range.  
...
Constant variance assumption is still getting violated per BP Test results and 'Fitted vs. Residual' plot. In future, we would have liked to further mitigate the issue by trying additional transformations such as Box-Cox of predictor variables. Similarly, Q-Q plot for top two models look much improved compared to when we started modeling with the untransformed response variable Time permitting, we would have liked to further investigate transformations  of the predictor variables to ensure we the assumption of normality.  
