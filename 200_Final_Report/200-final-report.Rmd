...
The first thing we we looked at were basic summary statistics and missingness. We found that there were 207 observation missing `total_bedrooms`. We cannot generate a linear model with missing data. However, looking at the colinearity, we will find a solution that allows us to keep these observations.
...
ggplot(my_sf) + 
  ...
  scale_colour_gradient2(low = "#009E73",
    midpoint = median(ca_housing_data$median_house_value),
    high = "#D55E00") 
```

We also generated histograms for all relevant predictors. The relevant most relevant finding of this EDA is the long tail of our outcome variable `median_house_value`.

```{r}
ggplot(ca_housing_data, aes(median_house_value)) +
  geom_histogram(fill = "#009E73", color= "#FFFFFF", bins = 15) +
...
pairs(ca_housing_data[,numeric_vals], col="#009E73")
...
In the table below, the reader can see this column has 5 different categories. It is important to note that the `ISLAND` category has only 5 observations. During initial analysis, our team determined that `ISLAND` has a substantially higher response variable `median_house_value` than the other categories as illustrated by the below boxplot. This fact is problematic for our purpose. We will be dropping the rows from data set with `ISLAND` category and coercing the predictor `ocean_proximity` to be categorical.
...
At this point, we have concluded the prerequisite data cleaning and can move onto model selection. Our final data set used for modelling has `r nrow(ca_housing_data)` and `r ncol(ca_housing_data)`. The summary statistics can be found below.
...
Now that we have data cleaned, we will proceed with building a model. The very first step we will take is to divide data into train and test. Based on total number of observations we have, team has decided to use 70%-30% split for train-test data.
...
Originally, we started with a simple additive linear model.... <!TODO: add more>

We then tried to reduce the model size using backwards AIC/BIC selection.... <!TODO: add more>

We also evaluated the impact of using interaction terms... <!TODO: add more>
