---
title: "STAT 420 Final Project Report: Modeling California Housing Prices"
author: " "
...
**Function   : **`get_model_diagnostic_tests` will generate model diagnostic tests such as:
                  B-P Test and Shapiro-Wilk test to check homoscedasticity and normality of residuals

**Inputs: **

*models      : * A list of models and model names to generate the results table

**Output     : ** Table with following diagnostic metrics as columns and a row for each model 
                  provided in `model_names` as input.
...
get_model_diagnostic_tests = function(models, model_names){
  ...
  for (idx in 1:n_mod){
    ...
    diagnostics_metrics_df[model_names[idx], "BP_DEC"] = ifelse(bp_pval < 0.05, 
                                                                "Reject", 
                                                                "Fail to Reject")
    ...
    diagnostics_metrics_df[model_names[idx], "SW_DEC"] = ifelse(sw_pval < 0.05, 
                                                                "Reject", 
                                                                "Fail to Reject")
  }
...
**Function   : **`get_model_diagnostic_plots` will generate model diagnostic plots such:
                  as Q-Q plot or fitted vs. residuals plot
...
**Output     : ** Plots showing diagnostic metrics 'Fitted vs. Residuals' & 'Q-Q  Plots' 
                  for our top two models.
...
get_model_diagnostic_plots = function(models, model_names){
  ...
  for (i in 1:n_mod){
    plot(fitted(models[[i]]), resid(models[[i]]), 
       ...
       xlab = "Fitted", 
       ylab = "Residuals", 
       main = paste0("Fitted vs. Residual for ", 
                     model_names[[i]])
       )
  ...
  par(mfrow=c(1,2))
  for (i in 1:n_mod){
    qqnorm(resid(models[[i]]), col = green_color, pch = 20,  
         main = paste0("Q-Q Plot for ", 
                       model_names[[i]]), 
         )
    qqline(resid(models[[i]]), col = "darkorange", lwd = 2)
  }
}
...
During the model building process, we explored **`r n_models`** different models for our housing data set. Recall that the main goal for this project was to effectively model the California Housing Data and identify the predictors which help explain the `median_house_value` response variable. Based on table [Comparative Model Performance Metrics](#comparative-model-performance-metrics) we select model `cahoudta_add_bckwd_bic_model` as our best model for purpose of this project. 
...
We also ran model diagnostic tests to check linear regression assumptions (LINE) with the help of `Breusch-Pagan Test` and `Shapiro-Wilk Test`. Decision of `Breusch-Pagan Test` for all the models is to reject the null hypothesis indicating violation of constant variable assumption (heteroscedasticity). Similarly, `Shapiro-Wilk Test` test indicates violation of normality assumption.  

```{r}
get_model_diagnostic_tests(cahoudta_models, cahoudta_model_names)
```

Residual vs Fitted plot and Q-Q plot for the two selected models 1) `cahoudta_full_add_model` and 2) `cahoudta_add_bckwd_bic_model` confirms the violation of assumptions for normality and constant variance. 
...
As noted above in section [Limitations of the Data](#limitations-of-the-data), our data is from the 1990 Census, which means we are not using this model for prediction but for explanation. Now that we have identified the best model with the available predictors, we can utilize the model to explain the relationship of geography to the `median_house_value` of a block group in California. The following section is useful to understand historical context of the California real estate market. We can also use this model to ask questions about the directionality and impact of the different predictors on `median_house_value`. In other words, our model allows us to answer question "Does a higher income block group have a higher average `median_house_value`?" 

### Interpretation of Model in Context of CA Block Groups in 1990s

There are noteworthy things about the below table of $\hat{\beta}$'s when discussing the model. The first is to note is that since we did a log transformation of the response, we will need to exponentiate the $\hat{\beta}$'s to correctly interpret the multiplicative effect of each of these parameters on `median_house_value`. For ease of interpretability, we have exponentiated the coefficients and then subtracted 1. This is because a coefficient less than 1 should be interpreted as a percent decrease in the average `median_house_value`, whereas a coefficient greater than 1 should be interpreted as a percent increase in the average `median_house_value`.

```{r}
coefs_exponentiated <- data.frame("Betas Exponentiated" = (exp(coef(final_selected_model)) - 1) * 100)
kable(coefs_exponentiated)
```
...
Please note that our base level for `ocean_proximity` is "<1H OCEAN". The implication is that the average `median_house_value` of a block group decreases for any other `ocean_proximity`. This is not surprising based on the above graphic where we see discrete clusters of high value houses in San Francisco and Los Angeles. The model found that blocks classified as "INLAND" had a `r abs(coefs_exponentiated["ocean_proximityINLAND",])`% decrease in average `median_house_value` relative to the blocks classified as "<1H OCEAN". 

The other two geographical variables, latitude and longitude, also have interesting interpretations based on our priors expecting geography to play a major role in price in 1990 CA block group house value. Specifically, for every 1 unit increase in latitude (moving Westward), we see a `r abs(coefs_exponentiated["latitude",])`% decrease in `median_house_value`. Similarly,  for every 1 unit increase in longitude (moving North), we see a `r abs(coefs_exponentiated["longitude",])`% decrease in `median_house_value`.

The other key $\hat{\beta}$ to call attention to is that for `median_income`. For every 1 unit increase in median income, we see `r abs(coefs_exponentiated["median_income",])`% increase in the `median_house_value`. In other words, the model found that blocks with higher incomes would have higher values of `median_house_value`. 

### Future Directions and Lessons Learned

It is also important to underscore future directions for analyses on this data set. Due to time constraints of this project our group was not able to implement some of these techniques. We also wanted to limit concepts and techniques from the course (STAT 420) while executing this final project. This section is meant to emphasize the careful considerations utilized by our team while building a simpler but effective model for the available data set.

Our team discussed about the idea of extracting meaningful Geo-Spatial insights utilizing variables derived from  `Latitude` and `Longitude`. The hypothesis being, house within 50 mile radius of a large city in California would likely have a higher median price. We considered leveraging Geo-Spatial libraries for this transformation activity. For example 'Haversine Formula' can be used to calculate distance between two points on a sphere based on latitude and longitude. Another alternative considered was to use Euclidean Distance. In the interest of time and to avoid scope creep, we decided not to create these variables. We think there is potential in future to transform this data and include additional Geo-Spatial variables to allow for better modeling. 

During exploratory data analysis we identified that median house prices histogram is showing an abnormal peak at the tail end of response variable `median_house_value`, rather than usual normal distribution long tail. It would be a good exercise in future to further explore this abnormality observed and transform data to allow for more accurate modeling. To address this it may require deleting observations with very high `median_house_value`, fit the model and run tests. 

The last model explored `cahoudta_2int_txfd_model` has the highest adjusted $R^2$ on training data but also a very high test RMSE which indicates overfitting. As a result we did not choose this model as one of possible solutions. It would be prudent to investigate if any other variable transformation mitigates overfitting, while still retaining high value for adjusted $R^2$. 

Constant variance assumption is still being violated as per BP Test results and 'Fitted vs. Residual' plot. It would be interesting to try additional transformations such as Box-Cox of predictor variables and see if resulting model is homoscedastic. Similarly, Q-Q plot for top two models track much better compared to models without log transformation of response variable `median_house_value`. 


## Appendix 

Below we have included a link to our GitHub repo.

### GitHub Repo for All Project Related Code

In preparation for this project, each of our team members conducted independent exploratory data analysis with separate .rmd files to understand data thoroughly. Additionally, team members engaged in model building worked through many iterations that included variable creation, iterating with `step()` search, and verifying the model assumptions. For the sake of brevity, we have not included all code used to select our model. However, we would like to leave a link to our GitHub repo where we all files and code developed in service of the project can be found.

The link to our GitHub repo is: [California House Price Prediction](https://github.com/katldewitt/house_price_prediction)
